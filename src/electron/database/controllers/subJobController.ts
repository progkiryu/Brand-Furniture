...
export const getAllSubJobs = async (
    ...
    try {
        ...
        if (!subJobs) {
            res.status(404).json({ message: "Error finding 'Subjobs' MongoDB collection!" });
        }        
...
export const getSubJobById = async (
    ...
    try {
        ...
        if (!id) {
            res.status(404).json({ message: `Failed to find sub-job with ID: ${id}`});
            return;
        }
...
export const updateSubJob = async (
    ...
    try {
        ...
        if (!result) {
            res.status(404).json({ message: `Failed to find sub-job with ID: ${id}! Or could not process request.`});
            return;
        }
        res.status(200).json(result).end();
    }
